...
-- Запрос 3 (доп с рекурсивным with)
WITH RECURSIVE "Иерархия_отделов" AS (
    SELECT "ИД", "КОРОТКОЕ_ИМЯ", "ОТД_ИД"
    FROM "Н_ОТДЕЛЫ"
    WHERE "КОРОТКОЕ_ИМЯ" = 'ФКТИУ'
    
    UNION ALL
   
    SELECT "о"."ИД", "о"."КОРОТКОЕ_ИМЯ", "о"."ОТД_ИД"
    FROM "Н_ОТДЕЛЫ" AS "о"
    JOIN "Иерархия_отделов" AS "иерархия"
    ON "о"."ОТД_ИД" = "иерархия"."ИД"
)
SELECT COUNT(*)
FROM "Н_УЧЕНИКИ" AS "ученики"
JOIN "Н_ПЛАНЫ" AS "планы" ON "ученики"."ПЛАН_ИД" = "планы"."ИД"
JOIN "Иерархия_отделов" AS "отделы" ON "планы"."ОТД_ИД" = "отделы"."ИД"
WHERE "ученики"."ИНН" IS NULL;
...
-- Запрос №4 (доп. с рекурсивным with)
WITH RECURSIVE "Иерархия_отделов" AS (
    SELECT "ИД", "КОРОТКОЕ_ИМЯ", "ОТД_ИД"
    FROM "Н_ОТДЕЛЫ"
    WHERE "КОРОТКОЕ_ИМЯ" = 'ФКТИУ'
...
-- Запрос №7 (доп. с рекурсивным with)
WITH RECURSIVE "Иерархия_отделов" AS (
    SELECT "ИД", "КОРОТКОЕ_ИМЯ", "ОТД_ИД"
    FROM "Н_ОТДЕЛЫ"
    WHERE "КОРОТКОЕ_ИМЯ" = 'ФКТИУ'
...
WHERE NOT EXISTS (
  ...
  FROM "Н_УЧЕНИКИ"
    JOIN "Н_ПЛАНЫ" ON "Н_УЧЕНИКИ"."ПЛАН_ИД" = "Н_ПЛАНЫ"."ИД"
    JOIN "Иерархия_отделов" ON "Н_ПЛАНЫ"."ОТД_ИД" = "Иерархия_отделов"."ИД"
  WHERE "Н_УЧЕНИКИ"."ЧЛВК_ИД" = "Н_ЛЮДИ"."ИД"
);
